...
function calculate() {
  ...
  // get input elements
  const totalLoan = document.getElementById("total-loan") as HTMLInputElement;
  ...
  if (totalLoan.value == "" || interest.value == "" || years.value == "") {
    ...
    const L: number = parseFloat(totalLoan.value);
    const I: number = parseFloat(interest.value) / 100; // convert percentage to decimal
    const Y: number = parseFloat(years.value) * 12; // convert years to months
